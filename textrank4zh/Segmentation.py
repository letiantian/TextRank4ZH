...
class WordSegmentation(object):
    ...
    def __init__(self, stop_words_file=None, allow_speech_tags=util.allow_speech_tags):
        ...
        f_out = codecs.open(self.stop_words_file, "r", "utf-8", "ignore")
        for word in f:
            self.stop_words.add(word.strip())
        f_out.close()

    def segment(
        self, text, lower=True, use_stop_words=True, use_speech_tags_filter=False
    ):
    ...
    def segment_sentences(
        self, sentences, lower=True, use_stop_words=True, use_speech_tags_filter=False
    ):
